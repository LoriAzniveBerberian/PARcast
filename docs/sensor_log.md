...
For the science behind the calibration values listed here, see [calibration_and_corrections.md](calibration_and_corrections.md).
...
| First calibration date (Apogee certificate) | _________________ |
| Calibration factor | 100.0 µmol m⁻² s⁻¹ per mV (manufacturer standard) |
| Wiring scheme | _____ White/Black/Clear (S/N ≥ 1559)  _____ Red/Black/Clear (S/N ≤ 1558) |
| Cable length | 5 m |
| Notes | |
...
| First calibration date (Apogee certificate) | _________________ |
| Calibration factor | 100.0 µmol m⁻² s⁻¹ per mV (manufacturer standard) |
| Immersion factor | _____ 1.25 (S/N ≥ 2876)   _____ 1.32 (S/N ≤ 2875) |
| Wiring scheme | _____ White/Black/Clear (S/N ≥ 1559)  _____ Red/Black/Clear (S/N ≤ 1558) |
| Cable length | 5 m |
| Notes | |
...
The model number and serial number are printed on a small label on the sensor cable, near the pigtail leads (see Apogee SQ-500 manual page 6). You may also find them on:

- The Apogee calibration certificate that shipped with each sensor (paper card or PDF)
- The receipt or order confirmation email from Apogee

If you can't find the serial numbers, contact Apogee customer support with your order number and they can look them up.
...
Tracking-only entries for accessories with serial-number traceability (mounting hardware, leveling plates, cable connectors).

| Item | Model | S/N or identifier | Notes |
|---|---|---|---|
| Solar sensor leveling plate | Apogee AL-100 | n/a | Used on E_s(PAR) reference station |
| | | | |
